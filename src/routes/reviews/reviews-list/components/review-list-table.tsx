...
import { reviewsLoader } from "../loader"
...
export const ProductListTable = () => {
  ...
  const initialData = useLoaderData() as Awaited<
    ReturnType<ReturnType<typeof reviewsLoader>>
  >

  const { searchParams, raw } = useProductTableQuery({ pageSize: PAGE_SIZE })
  const { products, reviews, count, isLoading, isError, error } = useReviews(
    {
      ...searchParams,
      is_giftcard: false,
    },
  ...
  console.log("products", products, "reviews", reviews)

  const filters = useProductTableFilters()
  const columns = useColumns()

  const { table } = useDataTable({
    data: (reviews ?? []) as HttpTypes.AdminProduct[],
    columns,
    count,
...
const useColumns = () => {
  const base = useReviewTableColumns()
